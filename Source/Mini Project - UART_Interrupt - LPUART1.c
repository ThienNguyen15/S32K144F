  ...
  while(1)
  {
		...
		if (Complete == 1)
		{
			...
			if(Check_Integer(result) != 0)
			{
				...
				strcat(print_result, "\r\n");
				LPUART1_send_string("\n");
				LPUART1_send_string(print_result);
				GPIOD->PCOR |= (1 << 16);
			}
			else
			{
...
{
	if ( (LPUART1->STAT & (1 << 20)) )    /* Wait for Idle Line is detected */
	{
		LPUART1->STAT |= (1 << 20);
	...
	else if (LPUART1->STAT & (1 << 21))    /* Wait for received buffer to be full */
	{
		State = 1;
		
		concatstr[index_concatstr] = (char)(LPUART1->DATA & 0x7F);
	}
	else if ( (LPUART1->STAT & (1 << 22) && State == 1 ) )    /* Wait for transmitter idle */
	{
		State = 0;
